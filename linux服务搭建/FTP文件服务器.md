...
## 九、案例 搭建一个部门的ftp  主管可上传，员工可下载



```shell 
#创建一个不能登录的用户 放再vuser_ftp这个目录下
useradd -s /sbin/nologin -d /var/tmp/vuser_ftp/ suibian
#修改 用户权限只有读和执行权限
chmod 500 /var/tmp/vuser_ftp
#创建四个部门  分别为1，2，3，4
mkdir /var/tmp/vuser_ftp/shichang
mkdir /var/tmp/vuser_ftp/xingzheng
mkdir /var/tmp/vuser_ftp/caiwu
mkdir /var/tmp/vuser_ftp/jishu

#修改vuser_ftp 下面的所有文件的权限为可读可写可执行
chmod 700 /var/tmp/vuser_ftp/*
#修改文件的所有者为suibian.suibian
chown suibian.suibian /var/tmp/vuser_ftp -R

#修改配置文件
#vim /etc/vsftpd/vsftpd.conf
cat >/etc/vsftpd/vsftpd.conf <<EOF
guest_enable=YES
guest_username=suibian
...
#修改vsftpd.conf  的chroot
vim /etc/vsftpd/vsftpd.conf


chroot_local_user=NO  #原本是YES

#然后去修改 chroot_list_file
